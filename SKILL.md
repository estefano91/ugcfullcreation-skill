...
| **Brand/product UGC — actor holding a real product with a specific label design** | NBP with the actual product photo as a SECOND reference image (not just a text description) | ~$0.15, same as normal NBP | Confirmed 2026-07-29/30 (isa-llopis, Lares Sugarcane Juice campaign). Describing a can's exact design in text only (colors, logo style, flavour text, badge) works for the first few shots but degrades over a multi-shot session — one shot dropped the sugarcane illustration and flavour text entirely. Fix: pass `image_urls: [identity_ref, product_photo_ref]` — two refs, one for the actor's face/identity, one an actual downloaded photo of the real product — and reference both explicitly in the prompt ("the woman in reference image 1... holding the exact drink can shown in reference image 2... matching that can label design exactly"). This measurably improved label fidelity (logo, sugarcane graphic, flavour text all present) versus the text-only approach, though very small badge text can still partially crop/blur depending on hand placement in the pose. When doing a full brand/product photo session, always fetch the real product image (WebFetch the brand site or download a product photo) and use it as a live visual ref for every shot, not just a one-time text description written from memory. |
